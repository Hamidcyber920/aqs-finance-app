...
- [x] Add scan receipt trigger tool (open_scanner — navigates to capture page and opens scanner)
...
- [x] Expose lockout status in admin panel (user list shows locked badge)
...
- [x] Strip PII (email, IP) from Sentry events in beforeSend hook
- [x] Add slow-request timing middleware (log > 2s tRPC calls)
...
- [x] Backend: localAuth.verifyTotp — exchange tempToken + TOTP code for full session
...
- [x] Frontend: Admin panel — show TOTP enabled badge on user list, allow admin to disable
