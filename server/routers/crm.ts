...
export const crmRouter = router({
  ...
  signGiftAidDeclaration: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const signedIp = (ctx.req as any).ip || "unknown";

      // Generate an HTML audit trail for the Gift Aid declaration and store in S3
      let certificateUrl: string | undefined;
      try {
        const pdfHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Gift Aid Declaration</title><style>body{font-family:Arial,sans-serif;margin:40px;color:#333}h1{color:#1B4332}table{width:100%;border-collapse:collapse;margin:16px 0}td{padding:8px;border:1px solid #ddd}td:first-child{font-weight:bold;width:180px}.footer{margin-top:40px;font-size:12px;color:#888}</style></head><body><h1>Gift Aid Declaration</h1><h2>Abdullah Quilliam Society</h2><table><tr><td>Donor Name</td><td>${input.donorName}</td></tr><tr><td>Address</td><td>${input.donorAddress}</td></tr><tr><td>Postcode</td><td>${input.donorPostcode}</td></tr><tr><td>Signature Method</td><td>${input.signatureMethod}</td></tr><tr><td>Signed At (UTC)</td><td>${signedAt.toISOString()}</td></tr><tr><td>IP Address</td><td>${signedIp}</td></tr><tr><td>Covers From</td><td>${input.coversFrom ?? "All past donations (4 years) and future donations"}</td></tr><tr><td>Covers To</td><td>${input.coversTo ?? "Until further notice"}</td></tr></table><h3>Declaration Text</h3><p>${GIFT_AID_DECLARATION_TEXT}</p><div class="footer">Electronic audit trail generated by the AQS management system. Legally equivalent to a signed paper declaration for HMRC Gift Aid purposes.</div></body></html>`;
        const buf = Buffer.from(pdfHtml, "utf-8");
        const fileKey = `gift-aid-certs/${input.donorId ?? input.donorLeadId ?? "anon"}-${signedAt.getTime()}.html`;
        const { url } = await storagePut(fileKey, buf, "text/html");
        certificateUrl = url;
      } catch (e) {
        console.error("[GiftAid] Failed to store certificate:", e);
      }
