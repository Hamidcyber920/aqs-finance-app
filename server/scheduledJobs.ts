...
export async function generateMorningBriefing() {
  ...
  try {
    ...
    const { systemSettings: sysSettingsTable, users: briefUsersTable } = await import("../drizzle/schema");
...
async function sendCalendarAndUrgentBriefing() {
  ...
  try {
    ...
    const db = await getDb();
    if (!db) return;
    const { systemSettings: sysSettings9am, users: briefUsers9am } = await import("../drizzle/schema");
